package com.pearson.qa.bluesky.csg.books.updateBookBuild.executors;
...
@Singleton
public class UpdateBookBuildExecutor<T extends UpdateBookBuildContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public UpdateBookBuildExecutor() {
        validators = new ArrayList<>();
    }

    public UpdateBookBuildExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new UpdateBookBuildResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<UpdateBookBuildContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<UpdateBookBuildContext> validator : validators)
