package com.pearson.qa.bluesky.csg.user.userResources.executors;
...
@Singleton
public class UserResourcesExecutor<T extends UserResourcesContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public UserResourcesExecutor() {
        validators = new ArrayList<>();
    }

    public UserResourcesExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new UserResourcesResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<UserResourcesContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<UserResourcesContext> validator : validators)
