package com.pearson.qa.bluesky.csg.books.commitOrderDetail.executors;
...
import com.pearson.qa.bluesky.csg.books.commitOrderDetail.datastructures.CommitOrderDetailResponsePayload;
...
@Singleton
public class CommitOrderDetailExecutor<T extends CommitOrderDetailContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public CommitOrderDetailExecutor() {
        validators = new ArrayList<>();
    }

    public CommitOrderDetailExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new CommitOrderDetailResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<CommitOrderDetailContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<CommitOrderDetailContext> validator : validators)
