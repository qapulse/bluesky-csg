package com.pearson.qa.bluesky.csg.books.schoolsByTerritory.executors;
 ...
 */
@Singleton
public class SchoolExecutor<T extends SchoolContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public SchoolExecutor() {
        validators = new ArrayList<>();
    }

    public SchoolExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new SchoolByTerritoryResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<SchoolContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<SchoolContext> validator : validators)
