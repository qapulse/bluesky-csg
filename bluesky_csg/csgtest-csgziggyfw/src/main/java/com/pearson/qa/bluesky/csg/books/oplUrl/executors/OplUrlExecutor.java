package com.pearson.qa.bluesky.csg.books.oplUrl.executors;
...
@Singleton
public class OplUrlExecutor<T extends OplUrlContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public OplUrlExecutor() {
        validators = new ArrayList<>();
    }

    public OplUrlExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new OplUrlResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<OplUrlContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<OplUrlContext> validator : validators)
