package com.pearson.qa.bluesky.csg.books.processCccOrder.contexts;

import com.google.inject.Inject;
import com.pearson.qa.bluesky.csg.Utils.datastructures.ErrorResponse;
import com.pearson.qa.bluesky.csg.books.processCccOrder.datastructures.ProcessCccOrderRequestPayload;
import com.pearson.qa.bluesky.csg.books.processCccOrder.datastructures.ProcessCccOrderResponsePayload;
import com.pearson.qa.common.configuration.Configuration;
import com.pearson.qa.common.ziggyfw.http.contexts.HttpContext;

/**
 * Created by didiercorrales on 3/9/14.
 */
public class  ProcessCccOrderContext extends HttpContext {
    ProcessCccOrderRequestPayload requestPayload;
    ProcessCccOrderResponsePayload responsePayload;
    ErrorResponse errorResponsePayload;

    public ProcessCccOrderContext() {
        super();
        setRequestPayload(new ProcessCccOrderRequestPayload());
        this.setRequestMethod("POST");
        this.setRequestContentType("application/json");
    }

    @Inject
    public void setConfiguration(Configuration configuration)
    {
        super.setConfiguration(configuration);
        this.configuration = configuration;
        if (this.configuration != null){
            String configNameSpace = configuration.getConfigNamespace(ProcessCccOrderContext.class);
            String scheme = configuration.getString(configNameSpace, "scheme", "http");
            this.setRequestScheme(scheme);
            String host = configuration.getString(configNameSpace, "host", "");
            this.setRequestHost(host);
            String path = configuration.getString(configNameSpace, "path", "set API url correctly");
            this.setRequestPath(path);
        }
    }


    public ProcessCccOrderRequestPayload getRequestPayload() {
        return requestPayload;
    }

    public void setRequestPayload(ProcessCccOrderRequestPayload requestPayload) {
        this.requestPayload = requestPayload;
    }

    public ProcessCccOrderResponsePayload getResponsePayload() {
        return responsePayload;
    }

    public void setResponsePayload(ProcessCccOrderResponsePayload responsePayload) {
        this.responsePayload = responsePayload;
    }

    public ErrorResponse getErrorResponsePayload() {
        return errorResponsePayload;
    }

    public void setErrorResponsePayload(ErrorResponse errorResponsePayload) {
        this.errorResponsePayload = errorResponsePayload;
    }
}
