package com.pearson.qa.bluesky.csg.collections.getCollectionNotes.executors;
...
@Singleton
public class GetCollectionNotesExecutor<T extends GetCollectionNotesContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public GetCollectionNotesExecutor() {
        validators = new ArrayList<>();
    }

    public GetCollectionNotesExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new GetCollectionNotesResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<GetCollectionNotesContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<GetCollectionNotesContext> validator : validators)
