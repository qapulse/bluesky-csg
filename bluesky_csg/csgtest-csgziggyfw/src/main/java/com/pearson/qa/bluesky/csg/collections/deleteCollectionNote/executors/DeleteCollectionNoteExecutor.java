package com.pearson.qa.bluesky.csg.collections.deleteCollectionNote.executors;
...
import com.pearson.qa.bluesky.csg.collections.deleteCollectionNote.datastructures.DeleteCollectionNoteResponsePayload;
...
@Singleton
public class DeleteCollectionNoteExecutor<T extends DeleteCollectionNoteContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public DeleteCollectionNoteExecutor() {
        validators = new ArrayList<>();
    }

    public DeleteCollectionNoteExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new DeleteCollectionNoteResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<DeleteCollectionNoteContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<DeleteCollectionNoteContext> validator : validators)
