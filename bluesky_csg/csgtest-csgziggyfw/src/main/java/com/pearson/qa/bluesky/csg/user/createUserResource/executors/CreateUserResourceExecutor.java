package com.pearson.qa.bluesky.csg.user.createUserResource.executors;
...
@Singleton
public class CreateUserResourceExecutor<T extends CreateUserResourceContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public CreateUserResourceExecutor() {
        validators = new ArrayList<>();
    }

    public CreateUserResourceExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new CreateUserResourceResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<CreateUserResourceContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<CreateUserResourceContext> validator : validators)
