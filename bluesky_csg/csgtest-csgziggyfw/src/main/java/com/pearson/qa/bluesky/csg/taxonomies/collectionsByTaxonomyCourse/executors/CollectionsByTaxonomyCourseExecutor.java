package com.pearson.qa.bluesky.csg.taxonomies.collectionsByTaxonomyCourse.executors;
...
@Singleton
public class CollectionsByTaxonomyCourseExecutor<T extends CollectionsByTaxonomyCourseContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public CollectionsByTaxonomyCourseExecutor() {
        validators = new ArrayList<>();
    }

    public CollectionsByTaxonomyCourseExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new CollectionsByTaxonomyCourseResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<CollectionsByTaxonomyCourseContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<CollectionsByTaxonomyCourseContext> validator : validators)
