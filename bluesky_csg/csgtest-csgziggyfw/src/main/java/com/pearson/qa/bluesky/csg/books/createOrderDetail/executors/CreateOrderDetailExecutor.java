package com.pearson.qa.bluesky.csg.books.createOrderDetail.executors;
 ...
 */
@Singleton
public class CreateOrderDetailExecutor<T extends CreateOrderDetailContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public CreateOrderDetailExecutor() {
        validators = new ArrayList<>();
    }

    public CreateOrderDetailExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new CreateOrderDetailResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<CreateOrderDetailContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<CreateOrderDetailContext> validator : validators)
