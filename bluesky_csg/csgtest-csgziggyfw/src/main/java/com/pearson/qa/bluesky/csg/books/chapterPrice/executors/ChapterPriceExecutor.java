package com.pearson.qa.bluesky.csg.books.chapterPrice.executors;
...
@Singleton
public class ChapterPriceExecutor<T extends ChapterPriceContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public ChapterPriceExecutor() {
        validators = new ArrayList<>();
    }

    public ChapterPriceExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new ChapterPriceResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<ChapterPriceContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<ChapterPriceContext> validator : validators)
