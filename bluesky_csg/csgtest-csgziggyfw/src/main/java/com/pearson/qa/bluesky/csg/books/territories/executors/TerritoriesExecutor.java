package com.pearson.qa.bluesky.csg.books.territories.executors;
...
@Singleton
public class TerritoriesExecutor<T extends TerritoriesContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public TerritoriesExecutor() {
        validators = new ArrayList<>();
    }

    public TerritoriesExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new TerritoriesResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<TerritoriesContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<TerritoriesContext> validator : validators)
