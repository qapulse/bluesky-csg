package com.pearson.qa.bluesky.csg.collections.updateResourceByResourceInstance.executors;
...
@Singleton
public class UpdateResourceByResourceInstanceExecutor<T extends UpdateResourceByResourceInstanceContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public UpdateResourceByResourceInstanceExecutor() {
        validators = new ArrayList<>();
    }

    public UpdateResourceByResourceInstanceExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new UpdateResourceByResourceInstanceResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<UpdateResourceByResourceInstanceContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<UpdateResourceByResourceInstanceContext> validator : validators)
