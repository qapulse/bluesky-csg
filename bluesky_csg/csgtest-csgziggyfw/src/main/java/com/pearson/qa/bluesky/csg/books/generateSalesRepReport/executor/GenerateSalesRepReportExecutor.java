package com.pearson.qa.bluesky.csg.books.generateSalesRepReport.executor;
...
@Singleton
public class GenerateSalesRepReportExecutor<T extends GenerateSalesRepReportContexts> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public GenerateSalesRepReportExecutor() {
        validators = new ArrayList<>();
    }

    public GenerateSalesRepReportExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new GenerateSalesRepReportResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<GenerateSalesRepReportContexts>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<GenerateSalesRepReportContexts> validator : validators)
