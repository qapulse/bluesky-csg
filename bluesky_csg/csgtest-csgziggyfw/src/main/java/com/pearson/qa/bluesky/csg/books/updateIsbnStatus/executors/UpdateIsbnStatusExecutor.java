package com.pearson.qa.bluesky.csg.books.updateIsbnStatus.executors;
...
@Singleton
public class UpdateIsbnStatusExecutor<T extends UpdateIsbnStatusContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public UpdateIsbnStatusExecutor() {
        validators = new ArrayList<>();
    }

    public UpdateIsbnStatusExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new UpdateIsbnStatusResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<UpdateIsbnStatusContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<UpdateIsbnStatusContext> validator : validators)
