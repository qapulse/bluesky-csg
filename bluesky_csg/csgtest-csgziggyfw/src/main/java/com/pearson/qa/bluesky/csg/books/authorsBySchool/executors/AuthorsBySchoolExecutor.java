package com.pearson.qa.bluesky.csg.books.authorsBySchool.executors;
...
import com.pearson.qa.bluesky.csg.books.authorsBySchool.datastructures.AuthorsBySchoolResponsePayload;
 ...
 */
@Singleton
public class AuthorsBySchoolExecutor<T extends AuthorsBySchoolContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public AuthorsBySchoolExecutor() {
        validators = new ArrayList<>();
    }

    public AuthorsBySchoolExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new AuthorsBySchoolResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<AuthorsBySchoolContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<AuthorsBySchoolContext> validator : validators)
