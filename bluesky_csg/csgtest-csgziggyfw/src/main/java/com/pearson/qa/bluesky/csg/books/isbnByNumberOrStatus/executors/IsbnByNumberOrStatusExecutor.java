package com.pearson.qa.bluesky.csg.books.isbnByNumberOrStatus.executors;
...
import com.pearson.qa.bluesky.csg.books.isbnByNumberOrStatus.datastructures.IsbnByNumberOrStatusResponsePayload;
...
@Singleton
public class IsbnByNumberOrStatusExecutor<T extends IsbnByNumberOrStatusContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public IsbnByNumberOrStatusExecutor() {
        validators = new ArrayList<>();
    }

    public IsbnByNumberOrStatusExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new IsbnByNumberOrStatusResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<IsbnByNumberOrStatusContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<IsbnByNumberOrStatusContext> validator : validators)
