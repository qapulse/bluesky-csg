package com.pearson.qa.bluesky.csg.collections.createCollectionNotes.executors;
...
@Singleton
public class CreateCollectionNotesExecutor<T extends CreateCollectionNotesContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public CreateCollectionNotesExecutor() {
        validators = new ArrayList<>();
    }

    public CreateCollectionNotesExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    if (!returnMessage.isEmpty())
                        context.setResponsePayload(new CreateCollectionNoteResponsePayload(returnMessage));
                    else
    ...
    @Inject
    public void setValidators(List<Validator<CreateCollectionNotesContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<CreateCollectionNotesContext> validator : validators)
