package com.pearson.qa.bluesky.csg.books.approveISBNs.executors;
...
@Singleton
public class ApproveIsbnExecutor<T extends ApproveIsbnContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public ApproveIsbnExecutor() {
        validators = new ArrayList<>();
    }

    public ApproveIsbnExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new ApproveIsbnResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<ApproveIsbnContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<ApproveIsbnContext> validator : validators)
