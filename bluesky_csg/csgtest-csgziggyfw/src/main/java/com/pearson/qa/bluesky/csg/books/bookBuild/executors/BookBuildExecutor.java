package com.pearson.qa.bluesky.csg.books.bookBuild.executors;
...
@Singleton
public class BookBuildExecutor<T extends BookBuildContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public BookBuildExecutor() {
        validators = new ArrayList<>();
    }

    public BookBuildExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new BookBuildResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<BookBuildContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<BookBuildContext> validator : validators)
