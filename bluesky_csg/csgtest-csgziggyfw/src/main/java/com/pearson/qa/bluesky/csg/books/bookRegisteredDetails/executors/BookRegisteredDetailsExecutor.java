package com.pearson.qa.bluesky.csg.books.bookRegisteredDetails.executors;
...
@Singleton
public class BookRegisteredDetailsExecutor<T extends BookRegisteredDetailsContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public BookRegisteredDetailsExecutor() {
        validators = new ArrayList<>();
    }

    public BookRegisteredDetailsExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new BookRegisteredDetailsResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<BookRegisteredDetailsContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<BookRegisteredDetailsContext> validator : validators)
