package com.pearson.qa.bluesky.csg.books.schoolTerritoryAssociation.executors;
...
@Singleton
public class SchoolTerritoryAssociationExecutor<T extends SchoolTerritoryAssociationContext> implements Executor<T> {
    private List<Validator<T>> validators;
    private HttpExecutor<T> httpExecutor;

    public SchoolTerritoryAssociationExecutor() {
        validators = new ArrayList<>();
    }

    public SchoolTerritoryAssociationExecutor(List<Validator<T>> validators, HttpExecutor<T> httpExecutor) {
        ...
        try {
            ...
            if (rawMessage != null && rawMessage.getClass() == JSONObject.class) {
                if (context.getReturnedHttpResponseCode() == 200) {
                    ...
                    context.setResponsePayload(new SchoolTerritoryAssociationResponsePayload(returnMessage));
                } else {
                    JSONObject returnMessage = (JSONObject) rawMessage;
    ...
    @Inject
    public void setValidators(List<Validator<SchoolTerritoryAssociationContext>> validators) {
        if (validators != null) {
            this.validators.clear();
            for (Validator<SchoolTerritoryAssociationContext> validator : validators)
