 ...
 */
public class PlayCollection extends TestBase {

    @Test(groups = {"collection.GetCollection"},
            dependsOnGroups = {"authentication.login.success"},
            dataProvider = "PlayCollectionDP",
            dataProviderClass = CollectionsDataProvider.class)

    public void PlayCollection(String CollectionIdDataProvider, String title, boolean isNegativeTest, int serverCode) {
        try {
            // prepare context
    ...
    @Test(groups = {"collection.PlayCollection"},
            dependsOnGroups = {"collection.createcollection"})
    public void PlayCollectionTestPositive() {
        try {
            // prepare context
            GetCollectionContext context = injector.getInstance(GetCollectionContext.class);
            Configuration configuration = injector.getInstance(Configuration.class);
